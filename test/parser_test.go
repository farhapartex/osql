...
func TestParseSpecExamples(t *testing.T) {
	tests := []struct {
		...
		{input: "select all from '~/Downloads'", target: query.TargetAll, path: "~/Downloads"},
		{input: "select files from 'Documents'", target: query.TargetFiles, path: "Documents"},
		{input: "select folders from '/'", target: query.TargetFolders, path: "/"},
		{input: "select files from Documents", target: query.TargetFiles, path: "Documents"},
		{input: "SELECT FILES FROM 'Documents';", target: query.TargetFiles, path: "Documents"},
		{
			input: "select files from 'Documents' where type = 'txt'", target: query.TargetFiles, path: "Documents",
		...
		{
			input: "select files from 'Documents' where name = 'notes.txt'", target: query.TargetFiles, path: "Documents",
			predicates: []query.Predicate{{Field: "name", Op: "=", Value: "notes.txt"}},
		},
		{
			input: "select files from 'Documents' where name != 'secret.txt'", target: query.TargetFiles, path: "Documents",
			predicates: []query.Predicate{{Field: "name", Op: "!=", Value: "secret.txt"}},
		},
		{
			input: "select files from 'Documents' where name_like = '%report%'", target: query.TargetFiles, path: "Documents",
		...
		{
			input: "select files from 'src' where name_like = 'test_%' and type = 'go'", target: query.TargetFiles, path: "src",
			predicates: []query.Predicate{
		...
		{
			input: "select folders from 'src' where count(child) > 10", target: query.TargetFolders, path: "src",
			predicates: []query.Predicate{{Field: "count(child)", Op: ">", Value: "10"}},
		},
		{
			input: "select folders from 'src' where count(child) = 0", target: query.TargetFolders, path: "src",
			predicates: []query.Predicate{{Field: "count(child)", Op: "=", Value: "0"}},
		},
		{
			input: "select folders from '~' where count(child) <= 2", target: query.TargetFolders, path: "~",
			predicates: []query.Predicate{{Field: "count(child)", Op: "<=", Value: "2"}},
		},
		{
			input: "select files from '~' recursive where name = 'notes.txt'", target: query.TargetFiles, path: "~", recursive: true,
			predicates: []query.Predicate{{Field: "name", Op: "=", Value: "notes.txt"}},
		},
		{
			input: "select files from '.' recursive where name_like = '%.log'", target: query.TargetFiles, path: ".", recursive: true,
...
func TestParseErrorKinds(t *testing.T) {
	tests := []struct {
		...
		{"unknown verb", "slect files from 'a'", oerr.KindUnknownVerb},
		{"empty input", "", oerr.KindUnknownVerb},
		{"missing target before from", "select from 'a'", oerr.KindMissingTarget},
		{"missing target at end", "select", oerr.KindMissingTarget},
		{"missing target before where", "select where name = 'a'", oerr.KindMissingTarget},
		{"singular file", "select file from 'a'", oerr.KindSingularTarget},
		{"singular folder", "select folder from 'a'", oerr.KindSingularTarget},
		{"unknown target", "select documents from 'a'", oerr.KindUnknownTarget},
		{"missing from", "select files 'a'", oerr.KindMissingFrom},
		{"missing from at end", "select files", oerr.KindMissingFrom},
		{"missing path", "select files from", oerr.KindMissingPath},
		{"missing path before where", "select files from where name = 'a'", oerr.KindMissingPath},
		{"unknown field", "select files from 'a' where extension = 'txt'", oerr.KindUnknownField},
		{"wrong operator for name", "select files from 'a' where name < 'b'", oerr.KindWrongOperator},
		{"wrong operator for type", "select files from 'a' where type > 'b'", oerr.KindWrongOperator},
		{"lone bang operator", "select files from 'a' where name ! 'b'", oerr.KindWrongOperator},
		{"count child on files", "select files from 'a' where count(child) > 1", oerr.KindCountChildOnFiles},
		{"count child non numeric", "select folders from 'a' where count(child) > 'many'", oerr.KindCountChildNonNumeric},
		{"unclosed quote", "select files from 'a", oerr.KindUnclosedQuote},
		{"trailing junk", "select files from 'a' junk", oerr.KindUnexpectedInput},
		{"junk after recursive", "select files from 'a' recursive junk", oerr.KindUnexpectedInput},
		{"where with nothing", "select files from 'a' where", oerr.KindIncompleteQuery},
		{"and with nothing", "select files from 'a' where name = 'b' and", oerr.KindIncompleteQuery},
		{"field with no operator", "select files from 'a' where name", oerr.KindIncompleteQuery},
		{"operator with no value", "select files from 'a' where name =", oerr.KindIncompleteQuery},
		{"or is not supported", "select files from 'a' where name = 'b' or type = 'c'", oerr.KindUnexpectedInput},
	}
...
func TestParseSingularTargetSuggestsPlural(t *testing.T) {
	_, err := parse(t, "select file from 'a'")
	if err == nil {
		t.Fatal("expected an error")
	}
	want := `Use "files", not "file" — for example: select files from 'Documents'`
	if err.Error() != want {
...
func TestParseUnknownFieldListsRegisteredFields(t *testing.T) {
	_, err := parse(t, "select files from 'a' where extension = 'txt'")
	if err == nil {
		t.Fatal("expected an error")
	}
...
func TestParseMalformedCountReportsUnknownField(t *testing.T) {
	tests := []string{
		"select folders from 'a' where count(parent) > 1",
		"select folders from 'a' where count = 1",
	}
...
func TestParseKeywordsAreCaseInsensitive(t *testing.T) {
	inputs := []string{
		"SELECT FILES FROM 'a' WHERE NAME = 'b'",
		"Select Files From 'a' Where Name = 'b'",
		"sElEcT fIlEs FrOm 'a' ReCuRsIvE",
		"select FOLDERS from 'a' where COUNT(CHILD) > 1",
	}
...
func TestParseNormalisesFieldNamesButNotValues(t *testing.T) {
	stmt, err := parse(t, "select files from 'a' where NAME = 'MixedCase.TXT'")
	if err != nil {
		t.Fatal(err)
	}
...
func TestParsePathPreservesCase(t *testing.T) {
	stmt, err := parse(t, "select files from '/Users/Nazmul/Documents'")
	if err != nil {
		t.Fatal(err)
	}
...
func TestParseRecursiveIsOptOut(t *testing.T) {
	stmt, err := parse(t, "select files from 'a'")
	if err != nil {
		t.Fatal(err)
	}
	if stmt.Recursive {
...
func TestParseRecursiveMustPrecedeWhere(t *testing.T) {
	if _, err := parse(t, "select files from 'a' where name = 'b' recursive"); err == nil {
		t.Error("recursive after where was accepted; the grammar places it before where")
	}
}

func TestParseBareWordValues(t *testing.T) {
	stmt, err := parse(t, "select files from 'a' where type = txt")
	if err != nil {
...
func TestParseTightOperatorSpacing(t *testing.T) {
	spaced, err := parse(t, "select files from 'a' where type = 'txt'")
	if err != nil {
		t.Fatal(err)
	}
	tight, err := parse(t, "select files from 'a' where type='txt'")
	if err != nil {
		t.Fatal(err)
	}
...
func TestParseThreePredicates(t *testing.T) {
	stmt, err := parse(t, "select files from 'a' where name_like = '%a%' and type = 'go' and name != 'b'")
	if err != nil {
		t.Fatal(err)
	}
...
func TestParsePredicateOrderIsPreserved(t *testing.T) {
	stmt, err := parse(t, "select files from 'a' where type = 'go' and name = 'b'")
	if err != nil {
		t.Fatal(err)
	}
...
func TestParseWithoutValidatorSkipsFieldChecks(t *testing.T) {
	stmt, err := parseNoValidation(t, "select files from 'a' where whatever = 'x'")
	if err != nil {
...
func FuzzParse(f *testing.F) {
	seeds := []string{
		...
		"select files from '~' recursive where name_like = '%a%'",
		"select",
		"",
		"select files from 'a' where",
		"select file from 'a'",
	}
	for _, s := range seeds {
		f.Add(s)
